...
> 即使縮到 17% 仍然高 780px，填充率掉到兩成）。**改比例（拉桿、輸入、拖角落）會
> 自動取消該元件的固定**：位置是在舊比例下挑的，留著只會讓重排縮不下去。
> 「還原原始版面」也**不**固定元件——恆等版面不靠固定維持（沒編輯就不會重排），
> 還原後再調整任何散圖，整張就照一般規則重新排到最小。
> 填充率偏低時右下角會直接提示有幾個元件被固定住——按「取消全部固定並重排」
> 解掉即可。
