...
| 操作 | 說明 |
|---|---|
| **整組比例** | 一拉就套用到這張合圖的所有元件（就是「調一整個群組」），並重新排到最小 |
| **拖曳角落** | 等比縮放選取的元件；可多選一起縮 |
| **拖曳元件** | 搬移，搬過的元件會被**固定**（黃框），自動排版時位置不動 |
| **單獨比例** | 選取後直接輸入百分比；比例不一致時顯示「多個比例」 |
| **重新排版** | MaxRects 裝箱找出最小頁面（可選最小尺寸 / 4 的倍數 / 2 的次方） |
| **取消全部固定並重排** | 版面縮不下去時的救命按鈕 |
| **間距** | 元件之間保留的透明像素，預設 2px |
| **還原原始版面** | 回到原檔的樣子：比例 100%、位置與頁面尺寸照原檔 |
| **移除自訂版面** | 這張合圖改回照全域比例整頁等比縮放 |

> **只有真的拖動位置才會「固定」元件**——單純點一下選取不會。被固定的元件不參與
> 自動排版，累積幾個之後畫布就縮不下去（例如一個固定在 y=760 的元件會讓整張圖
> 即使縮到 17% 仍然高 780px，填充率掉到兩成）。所以「整組比例」會一併取消所有
> 固定再重排——那些位置本來就是在別的比例下挑的。填充率偏低時右下角也會直接
> 提示有幾個元件被固定住。
...
| | 輸出 | 全域縮放比例 |
|---|---|---|
| **還原原始版面** | atlas 與原檔 **byte-identical**、貼圖像素不變（只做壓縮） | **不生效**（等於「這張合圖不要動」） |
| **移除自訂版面** | 整頁等比縮放，版面配置與原本相同 | 生效 |

所以要把某一張合圖排除在整批縮放之外，用「還原原始版面」；
要它回去跟著大家一起縮，用「移除自訂版面」。
...
同一個尺寸。檢視也只在整體操作（整組比例、間距、對齊、重新排版）後才重新置中，
...
「已套用 50% ＋合圖」（只有部分頁面有版面），容量變化欄與中間的播放預覽
也會立刻跟著更新。
