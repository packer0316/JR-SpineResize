...
| 功能 | 說明 |
|---|---|
| **儲存專案** | 把整個清單與各專案的設定存成 `.jrspine` 專案檔。**只記錄素材的絕對路徑與設定，不含任何圖片資料**，所以檔案通常只有幾 KB。 |
| **開啟專案** | 依專案檔記錄的絕對路徑重新掃描素材並還原設定。素材被搬走或刪除時不會失敗，只會回報哪些找不到。 |
...
| 裁切區塊 | 支援 |
| 多頁 atlas | 支援 |
| 多個 atlas 共用同一張貼圖 | 支援，同批次只渲染一次，其餘 atlas 沿用結果（覆蓋原檔模式也不會誤判成二次縮放）；區塊版面真的不同時才會報錯 |
| 貼圖格式 | PNG / WebP / JPG |
...
│   └── spine_asset.py
├── core/
│   ├── project_file.py        # .jrspine 專案檔（只存絕對路徑與設定）
