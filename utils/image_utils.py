...
try:  # pngquant 核心，與 JR-Img-Compresser 使用同一套
    import imagequant

    HAS_IMAGEQUANT = True
except ImportError:  # pragma: no cover - 取決於安裝環境
...
def quantize_to_palette(
    ...
    """
    把 RGBA 影像量化成 8-bit 調色盤（保留 alpha），回傳 (影像, 使用的演算法)。

    優先使用 imagequant（pngquant 核心），品質明顯優於 Pillow 內建的
    Fast Octree；沒安裝時才退回 Pillow。

    Pillow 的 MEDIANCUT 不支援 RGBA，所以退路只有 FASTOCTREE。
    """
    if image.mode != "RGBA":
        image = image.convert("RGBA")
    ...
    if HAS_IMAGEQUANT:
        try:
            quantized = imagequant.quantize_pil_image(
                image,
                dithering_level=max(0.0, min(1.0, dithering)),
                max_colors=max(2, min(256, colors)),
                min_quality=0,
                max_quality=100,
            )
            return quantized, "imagequant"
        except Exception:  # pragma: no cover - 量化失敗時仍要能輸出
            pass

    fallback = image.quantize(
        colors=max(2, min(256, colors)), method=Image.Quantize.FASTOCTREE
    )
    return fallback, "fastoctree"
